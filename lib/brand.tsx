...
export function MediCrewLogo({compact=false}:{compact?:boolean}) {
  return <View style={[styles.row, compact&&styles.compactRow]}>
    <LinearGradient colors={[colors.greenStart, colors.green, colors.greenEnd]} start={{x:0,y:1}} end={{x:1,y:0}} style={[styles.mark,compact&&styles.compactMark]}>
      <Text style={[styles.m,compact&&styles.compactM]}>M</Text>
      <View style={[styles.plusBox,compact&&styles.compactPlusBox]}><Text style={[styles.plus,compact&&styles.compactPlus]}>+</Text></View>
    </LinearGradient>
    {!compact&&<Text style={styles.wordmark}>Medi<Text style={styles.wordmarkCrew}>Crew</Text></Text>}
  </View>;
}
