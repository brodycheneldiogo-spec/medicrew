-- MediCrew: both sides confirm completion before a mission becomes completed.
...
begin
  select m.company_id, m.status, a.professional_id,
         a.company_completed_at, a.professional_completed_at
    into v_company_id, v_status, v_professional_id,
         v_company_done, v_professional_done
  ...
  if auth.uid() <> v_company_id and auth.uid() <> v_professional_id then
    raise exception 'Not authorized for this mission';
  end if;

  if v_status <> 'in_progress' then
    raise exception 'Only in-progress missions can be completed';
  end if;

  if v_professional_id is null then
    raise exception 'Mission has no assigned professional';
  end if;

  if auth.uid() = v_company_id then
    update public.mission_assignments
      set company_completed_at = coalesce(company_completed_at, now())
  ...
  if v_company_done is not null and v_professional_done is not null then
    ...
    insert into public.notifications(profile_id, title, body, type, data)
    values
      (case when auth.uid() = v_company_id then v_professional_id else v_company_id end,
       'Completion confirmed',
       'The other party confirmed completion. Please confirm when your side is complete.',
       'mission_completion_pending',
  ...
  values (
    auth.uid(),
    case when v_next = 'completed' then 'mission_completed' else 'mission_completion_confirmed' end,
    'mission',
    p_mission_id,
...
-- The generic status RPC must no longer allow one party to mark a mission completed alone.
...
begin
  ...
  from public.mission_assignments where mission_id = p_mission_id;
  ...
  insert into public.audit_logs(actor_id, action, entity_type, entity_id, metadata)
  values (auth.uid(), 'mission_status_changed', 'mission', p_mission_id, jsonb_build_object('status', p_next_status));
