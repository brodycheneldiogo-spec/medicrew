...
begin
  ...
  if not exists (
    ...
    where id = p_professional_id and verification_status = 'verified'
  ) then
  ...
  on conflict (mission_id) do update
    set professional_id = excluded.professional_id, confirmed_at = now()
  ...
  insert into public.conversations (mission_id)
  values (p_mission_id)
...
-- Controlled mission status transitions. This prevents arbitrary client-side jumps.
...
begin
  ...
  if p_next_status='in_progress' then
    if auth.uid() <> v_company_id and auth.uid() <> v_assigned then raise exception 'Not authorized'; end if;
    if v_current <> 'confirmed' then raise exception 'Only confirmed missions can start'; end if;
  elsif p_next_status='completed' then
    if v_current <> 'in_progress' then raise exception 'Only in-progress missions can be completed'; end if;
  ...
  if v_assigned is not null then
    ...
    values
      (case when auth.uid()=v_company_id then v_assigned else v_company_id end,
       case p_next_status when 'in_progress' then 'Mission started' when 'completed' then 'Mission completed' else 'Mission cancelled' end,
       case p_next_status when 'in_progress' then 'The MediCrew mission is now in progress.' when 'completed' then 'The MediCrew mission has been marked completed.' else 'The MediCrew mission has been cancelled.' end,
