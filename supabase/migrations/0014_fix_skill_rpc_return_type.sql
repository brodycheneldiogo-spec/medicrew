-- Fix replace_my_skills without changing the return type of the existing function.
-- PostgreSQL does not allow CREATE OR REPLACE FUNCTION to change RETURNS void -> jsonb.
-- Keep the original void return type and make the write itself deterministic.
...
begin
  ...
  for item in
    select * from jsonb_array_elements(coalesce(p_skills, '[]'::jsonb))
  loop
    ...
    begin
      v_level = coalesce(
        nullif(item->>'level', '')::public.experience_level,
        'basic'
      );
