-- MediCrew production consolidation.
-- This migration reconciles the production schema after the historical 0015/0016/0017/0027/0028
-- duplicate migration numbering. It is intentionally idempotent so it can be applied to the current
-- production schema after the corresponding historical versions have been repaired as applied.

-- ---------- Account / international metadata ----------
...
drop constraint if exists missions_mission_kind_check on public.missions;
...
create index if not exists missions_kind_country_idx
  on public.missions(mission_kind,event_country,departure_at);
create index if not exists professionals_country_idx
  on public.professionals(country_of_operation);
create index if not exists availability_professional_starts_idx
...
create policy push_tokens_owner_select on public.push_tokens for select to authenticated
...
create policy push_tokens_owner_delete on public.push_tokens for delete to authenticated
  using (profile_id = auth.uid());
...
create index if not exists notification_push_queue_pending_idx
  on public.notification_push_queue(created_at) where delivered_at is null;
...
create policy notification_push_queue_owner_select on public.notification_push_queue for select to authenticated
  using (profile_id = auth.uid());
...
create index if not exists company_verification_documents_company_idx
  on public.company_verification_documents(company_id,created_at desc);
...
drop trigger if exists company_verification_documents_updated_at on public.company_verification_documents;
create trigger company_verification_documents_updated_at before update on public.company_verification_documents
  for each row execute function public.set_updated_at();
...
begin
  ...
  select jsonb_build_object(
    'companies', coalesce((select jsonb_agg(row_to_json(x) order by x.created_at desc)
      from (
        select c.id,c.company_name,c.siren,c.siret,c.organization_type,c.verification_status,
               c.created_at,c.updated_at,p.first_name,p.last_name,p.email,p.phone
        from public.companies c join public.profiles p on p.id=c.id
        where c.verification_status='pending'
      ) x),'[]'::jsonb),
    'documents', coalesce((select jsonb_agg(row_to_json(y) order by y.created_at desc)
      from (
        select d.id,d.company_id,d.document_type,d.title,d.reference,d.storage_path,d.original_name,
               d.mime_type,d.size_bytes,d.status,d.rejection_reason,d.expires_at,d.uploaded_at,
               c.company_name,p.email
...
create policy mission_payments_admin_read on public.mission_payments for select to authenticated
  using (exists(select 1 from public.profiles p where p.id=auth.uid() and p.role='admin'));
drop trigger if exists mission_payments_updated_at on public.mission_payments;
create trigger mission_payments_updated_at before update on public.mission_payments
  for each row execute function public.set_updated_at();
...
begin
  ...
  on conflict(mission_id) do update set
    company_id=excluded.company_id,professional_id=excluded.professional_id,amount_cents=excluded.amount_cents,
    platform_fee_cents=excluded.platform_fee_cents,professional_amount_cents=excluded.professional_amount_cents,
  ...
  return query select mp.id,mp.amount_cents,mp.platform_fee_cents,mp.professional_amount_cents,mp.currency,mp.status
    from public.mission_payments mp where mp.id=v_payment;
end;
$$;
...
begin
  select m.company_id,m.status,a.professional_id into v_company,v_status,v_professional
  from public.missions m join public.mission_assignments a on a.mission_id=m.id where m.id=p_mission_id;
...
begin
  select company_id,event_country into v_company_id,v_country from public.missions
    where id=p_mission_id and mission_kind='event';
  ...
  select m.id,p.id,
    round((case when p.verification_status='verified' then 30 else 0 end
    ...
    p.verification_status='verified' and p.professional_type=m.professional_type
  ...
  select mm.professional_id,'New event mission near you',
...
-- If the intended admin profile already exists, promote it. This is deliberately limited to the
-- exact identifiers supplied for the MediCrew admin account; no password or secret is stored here.
update public.profiles set role='admin',updated_at=now()
where lower(coalesce(email,''))='ethanbrody@gmail.com' or regexp_replace(coalesce(phone,''),'[^0-9+]','','g')='0674356279';
...
comment on table public.mission_payments is 'Server-controlled Stripe payment ledger for company-funded missions. Stripe secrets must never be stored in the mobile client.';
