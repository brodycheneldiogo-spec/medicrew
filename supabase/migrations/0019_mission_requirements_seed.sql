-- Add a reusable catalog of requirement semantics without storing patient information.
-- Mission requirements remain limited to operational/professional competency data.
...
begin
  ...
  insert into public.mission_requirements(mission_id,skill_id,required,minimum_level,minimum_years)
  values(p_mission_id,v_skill,p_required,p_minimum_level,greatest(p_minimum_years,0))
  on conflict(mission_id,skill_id) do update set required=excluded.required,minimum_level=excluded.minimum_level,minimum_years=excluded.minimum_years
