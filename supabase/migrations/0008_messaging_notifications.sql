...
returns table (conversation_id uuid, mission_id uuid, mission_title text, other_profile_id uuid, other_first_name text, other_last_name text, last_message text, last_message_at timestamptz, unread_count bigint)
language sql security definer set search_path = public
as $$
  select c.id,c.mission_id,m.title,other_p.id,other_p.first_name,other_p.last_name,lm.body,lm.created_at,
  ...
  from public.conversations c join public.missions m on m.id=c.mission_id
  join public.conversation_members mine on mine.conversation_id=c.id and mine.profile_id=auth.uid()
  left join lateral (select p.* from public.conversation_members cm join public.profiles p on p.id=cm.profile_id where cm.conversation_id=c.id and cm.profile_id<>auth.uid() limit 1) other_p on true
...
as $$
declare v_conversation uuid; v_message uuid; v_recipient uuid; v_clean text:=trim(coalesce(p_body,''));
begin
...
as $$ begin
 if not exists(select 1 from public.conversation_members where conversation_id=p_conversation_id and profile_id=auth.uid()) then raise exception 'Not a conversation member'; end if;
...
create policy "own conversation membership read" on public.conversation_members for select using (profile_id=auth.uid());
