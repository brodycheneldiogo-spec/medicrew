...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const{data:mission,error:missionError}=await admin.from('missions').select('id,company_id,status,compensation_cents,service_fee_cents,title,destination_location').eq('id',missionId).maybeSingle();
    ...
    const item=await stripe.invoiceItems.create({customer:customer.id,currency:'eur',amount:mission.service_fee_cents,description:`MediCrew platform service fee · mission ${mission.id}`},{idempotencyKey:`medicrew-invoice-item-${missionId}`});
    const invoice=await stripe.invoices.create({customer:customer.id,collection_method:'send_invoice',days_until_due:7,auto_advance:true,description:`MediCrew service fee for mission ${mission.id}`,metadata:{medicrew_mission_id:mission.id,medicrew_company_id:user.id,professional_fee_cents:String(mission.compensation_cents),service_fee_cents:String(mission.service_fee_cents)}},{idempotencyKey:`medicrew-invoice-${missionId}`});
    const saved=await admin.from('mission_invoices').upsert({mission_id:mission.id,company_id:user.id,stripe_customer_id:customer.id,stripe_invoice_id:invoice.id,status:invoice.status==='open'?'open':'draft',amount_cents:mission.service_fee_cents,currency:'eur',hosted_invoice_url:invoice.hosted_invoice_url||null,invoice_pdf_url:invoice.invoice_pdf||null,due_at:invoice.due_date?new Date(invoice.due_date*1000).toISOString():null,updated_at:new Date().toISOString()},{onConflict:'mission_id'}).select('*').single();
