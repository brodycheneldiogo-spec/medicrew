...
function VerificationAccessGuard(){const segments=useSegments();useEffect(()=>{let alive=true;const check=async()=>{const c=supabase;if(!c)return;const{data:{user}}=await c.auth.getUser();if(!user||!alive)return;const path=segments.join('/');if(EXEMPT.has(path))return;const{data,error}=await c.rpc('my_account_access_state');if(error||!alive)return;const state=(data||{}) as {role?:string;status?:string;allowed?:boolean};if(state.role==='admin')return;if(state.allowed===false)router.replace('/pending-review' as never)};void check();return()=>{alive=false}},[segments]);return null}

export default function RootLayout(){return <PreferencesProvider><StatusBar barStyle="dark-content" backgroundColor={colors.paper}/><NotificationObserver/><VerificationAccessGuard/><Stack screenOptions={{headerShown:false,contentStyle:{backgroundColor:colors.paper},animation:'slide_from_right',gestureEnabled:true,fullScreenGestureEnabled:true}}/></PreferencesProvider>}
