...
export default function CompanyVerification() {
  ...
  async function addDocument() {
    ...
    if (file.size != null && file.size > MAX_BYTES)
      return setError(
        L(
          ...
          "El archivo debe pesar 10 MB como máximo.",
        ),
      );
    ...
    if (!ALLOWED_TYPES.includes(mime))
      return setError(
        L(
          ...
          "Choisissez un fichier PDF, JPG, PNG ou WEBP.",
          "Elige un archivo PDF, JPG, PNG o WEBP.",
        ),
      );
    setSaving(true);
    let storagePath = "";
    try {
      ...
      const safeName =
        file.name.replace(/[^a-zA-Z0-9._-]/g, "_").slice(-100) || "document";
      ...
      const row = await supabase.from("company_verification_documents").insert({
        company_id: user.id,
      ...
      setError(
        e?.message ||
          L(
            ...
            "No se pudo añadir el documento.",
          ),
      );
    } finally {
      setSaving(false);
    ...
    <SafeAreaView style={s.safe}>
      <ScrollView contentContainerStyle={s.container}>
        <View style={s.content}>
          ...
          <Text style={s.sub}>
            {L(
              "No specific document is required. Add any document that helps the MediCrew team verify your organization. It stays private.",
              "Aucun document précis n’est imposé. Ajoutez simplement un document permettant à l’équipe MediCrew de vérifier votre entreprise. Il reste privé.",
              "No se exige ningún documento específico. Añade un documento que ayude al equipo de MediCrew a verificar tu empresa. Permanecerá privado.",
            )}
          </Text>
          {error ? (
            <View style={s.error}>
          ...
          <View style={s.card}>
            ...
            <Text style={s.formats}>PDF · JPG · PNG · WEBP · 10 MB max</Text>
            <Pressable
              disabled={saving}
              onPress={addDocument}
              style={[s.button, saving && { opacity: 0.65 }]}
            >
              {saving ? (
...
const s = StyleSheet.create({
  ...
  formats: { fontSize: 11, color: colors.muted, marginTop: 5 },
  button: {
    height: 54,
