...
export default function EditProfessionalProfile() {
  ...
  async function load() {
    try {
      ...
      const [{ data: p, error: pe }, { data: pro, error: pre }] = await Promise.all([
        ...
        supabase.from('professionals').select('professional_type,date_of_birth,address,nationality,specialty,rpps_number,years_experience,medical_transport_years,air_ambulance_years,repatriation_years,emergency_years,icu_years').eq('id', user.id).single(),
      ]);
      if (pe) throw pe; if (pre) throw pre;
      const next: Record<string,string> = {};
      for (const f of fields) next[f[0]] = String((p as any)?.[f[0]] ?? (pro as any)?.[f[0]] ?? '');
      setForm(next); setType(pro.professional_type);
    } catch (e:any) { Alert.alert('Profile unavailable', e?.message || 'Unable to load profile'); router.back(); }
  ...
  async function save() {
    ...
    try {
      ...
      const professionalPayload:any = { professional_type:type, address:form.address?.trim()||null, nationality:form.nationality?.trim()||null, specialty:form.specialty?.trim()||null, rpps_number:form.rpps_number?.trim()||null };
      for (const key of ['years_experience','medical_transport_years','air_ambulance_years','repatriation_years','emergency_years','icu_years']) {
        const n = Number(form[key] || 0); if (!Number.isFinite(n) || n < 0) throw new Error(`Invalid value for ${key.replaceAll('_',' ')}`); professionalPayload[key] = n;
      }
      const { error: pe } = await supabase.from('profiles').update(profilePayload).eq('id', user.id);
      if (pe) throw pe;
      const { error: pre } = await supabase.from('professionals').update(professionalPayload).eq('id', user.id);
      if (pre) throw pre;
      Alert.alert('Saved', 'Your professional profile has been updated.', [{text:'Done',onPress:()=>router.back()}]);
    } catch (e:any) { Alert.alert('Save failed', e?.message || 'Unable to save profile'); }
