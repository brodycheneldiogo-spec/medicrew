...
export default function Onboarding() {
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.container}>
    ...
    <View>
      ...
      {!professional&&step===0&&<><Text style={styles.label}>Legal company name</Text><TextInput value={name} onChangeText={setName} placeholder="Company name" placeholderTextColor="#9AA19F" style={styles.input}/><Text style={[styles.label,{marginTop:14}]}>Organization type</Text><View style={styles.wrap}>{organizationTypes.map(([id,label])=><Pressable key={id} onPress={()=>setOrganizationType(id)} style={[styles.tag,organizationType===id&&styles.tagActive]}><Text style={[styles.tagText,organizationType===id&&styles.tagTextActive]}>{label}</Text></Pressable>)}</View></>}
